function [lny,sigma,tau,phi]=Kuehn2020(To,M,Rrup,Ztor,Vs30,Z10,Z25,mechanism,region,slab,aBackarc,aNankai,Mb,Basin,AleInSigma,EpiInSigma,Nsample)
...
if T_lo==T_hi
    [lny,sigma,tau,phi]=gmpe(index,M,Rrup,Ztor,Vs30,Z10,Z25,mechanism,region,slab,aBackarc,aNankai,Mb,Basin,AleInSigma,EpiInSigma,Nsample);
else
    [lny_lo,~,tau_lo,phi_lo] = gmpe(index,   M,Rrup,Ztor,Vs30,Z10,Z25,mechanism,region,slab,aBackarc,aNankai,Mb,Basin,AleInSigma,EpiInSigma,Nsample);
    [lny_hi,~,tau_hi,phi_hi] = gmpe(index+1, M,Rrup,Ztor,Vs30,Z10,Z25,mechanism,region,slab,aBackarc,aNankai,Mb,Basin,AleInSigma,EpiInSigma,Nsample);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
