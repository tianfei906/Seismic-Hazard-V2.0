function[lny,sigma,tau,phi]=BSSA2014(To,M,Rjb,Vs30,BasinDepth,SOF,region)
...
if T_lo==T_hi
    [lny,sigma,tau] = gmpe(index,M, Rjb, SOF, region, BasinDepth, Vs30);
    phi             = sqrt(sigma.^2-tau.^2);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M, Rjb, SOF, region, BasinDepth, Vs30);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M, Rjb, SOF, region, BasinDepth, Vs30);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
