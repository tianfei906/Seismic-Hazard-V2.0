function[lambdaD]=psda_BMT2017_cdmM(Ts_, ky_, param,im,M,dPm,Haz50)
...
for m=1:Nmag
    for j=1:Nim
        ...
        lnd(j,m,TH) = -6.896-3.353*log(ky(TH))-0.390*(log(ky(TH))).^2+0.538*log(ky(TH))*log(im(j))+3.060*log(im(j))-0.225*(log(im(j))).^2+3.801*Ts(TH)+0.55*M(m)-0.803*Ts(TH).^2;
    end
end
