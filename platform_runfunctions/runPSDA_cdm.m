...
for site_ptr=1:Nsites
    ...
    for model_ptr=1:Nmodels
        ...
        ind        = selectsource(opt0.maxdist,r0,allsource);
        ind        = find(ind);
        ...
        for source_ptr=ind
            ...
            if integrator==5
                ...
                loc_param.hazard      = SMLIB(Bs).param{1};
                loc_param.integration = SMLIB(Bs).param{2};
                
                switch  loc_param.integration
                    case 'PC', [MRE,Cz] = runPCE(source,r0,IMslope,im,realSa,optL,h.value);
                    case 'MC', [MRE,Cz] = runMCS(source,r0,IMslope,im,realSa,optL,h.value);
                end
            ...
            if integrator==6
                ...
                loc_param.integration = SMLIB(Bs).param{2};
                im       = handles.haz2.im1;
                MRE      = runPCE(source,r0,IMslope,im,realSa,optL,h.value);
                MRE      = permute(MRE,[1 3 2]);
                Haz50    = prctile(MRE,50,2);
                deagg    = handles.haz2.deagg1(site_ptr,:,:,source_ptr);
                [M,dPm]  = getMdPm(deagg);
