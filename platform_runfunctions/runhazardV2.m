...
if nargin==7
    if opt.method==1
        for i=sptr
            [lambda(:,:,i),MRE(:,:,i),MRD(:,:,i)]=runsource1(source(i),im,IM,rho,xyz,media,opt);
        end
    end
    
    if opt.method==2
        for i=sptr
            [lambda(:,:,i),MRE(:,:,i),MRD(:,:,i)]=runsource2(source(i),im,IM,rho,xyz,media,opt);
        end
    end
    
    if opt.method==3
        for i=sptr
            [lambda(:,:,i),MRE(:,:,i),MRD(:,:,i)]=runsource3(source(i),im,IM,rho,xyz,media,h,opt);
        end
    end
    lambda = sum(lambda,3,'omitnan');
    MRE    = sum(MRE   ,3,'omitnan');
    MRD    = sum(MRD   ,3,'omitnan');
end
...
function[lambda,MRE,MRD]=runsource1(source,im,IM,rho,xyz,media,opt)
...
nsig   = 6;
strunc = opt.strunc;
if strunc(1)==1 %'overwrite'
    sig1=sig1.^strunc(2)*strunc(3);
    sig2=sig2.^strunc(2)*strunc(3);
end
...
if all(sig1==0) && all(sig2==0)
    ...
    MRD   = NMmin*sum(fSa1.*fSa2.*rate,3,'omitnan');  % eq 3
    MRE   = zeros(Nim,Nim);
...
for i=1:Nim0
    ...
    ccdf2   = 1-cdf(pd,xhat);
    deagg2  = ccdf2.*rate;
    lambda(i,:) = NMmin*sum([deagg1,deagg2],3,'omitnan');
end

return

function[lambda,MRE,MRD]=runsource2(source,im,IM,rho,xyz,media,opt)
...
function[lambda,MRE,MRD]=runsource3(source,im,IM,rho,xyz,media,h,opt)
...
%% MRD using mvnpdf
[mu1,sig1] = gmpe.handle(IM(1),param{:});
[mu2,sig2] = gmpe.handle(IM(2),param{:});
...
if opt.strunc(1)==1
    sig1=sig1*opt.strunc(2);
    sig2=sig2*opt.strunc(2);
end
