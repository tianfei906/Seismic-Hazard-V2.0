...
class Element:
    def __init__(self, data, coord, stencil=None, standardize=False, normalize=True):
        ...
        if self.phyDim == 1:
            ...
            temp_cliu = np.array([np.arange(coord[0], coord[1]+delta_coord, delta_coord)]).T  # an n-by-one matrix
            if temp_cliu.shape[0] > self.shape[0]:
                temp_cliu = temp_cliu[0:-1,:]
            self.coords = temp_cliu
    ...
    def calc_like_energy(self, mu, cov):
        ...
        for l in range(self.n_labels):
            le[:, l] = np.einsum("...i,ji,...j",
                                 ...
                                 np.linalg.inv(cov[l, :, :] + np.eye(self.n_feat) * 1e-6),
    ...
    def fit(self, num_of_iter, n_labels, beta_init=1, beta_jump_length=0.1, mu_jump_length=0.05,
        ...
        if prior_mu_cov is None:
            ...
            prior_mu_covs = [np.eye(self.n_feat) * 100] * self.n_labels
        else:
...
def _propose_cov(cov_prev, n_feat, n_labels, cov_jump_length, theta_jump_length):
    ...
    theta_jump = multivariate_normal(mean=[0] * n_comb, cov=np.ones(n_comb) * theta_jump_length).rvs()
    ...
    for l in range(n_labels):
        ...
        log_d_jump = multivariate_normal(mean=[0] * n_feat, cov=np.eye(n_feat) * cov_jump_length).rvs()
