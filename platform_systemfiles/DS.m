...
function[]=ds_default(handles,val,filename)
...
switch val
    ...
    case 7
        ...
        if nargin==2
            [ff,pp] = uigetfile('*.txt');
            filename =fullfile(pp,ff);
        end
        warning on
        edi(2).Value=1;
        edi(2).String={filename};
...
switch func2str(fun)
    ...
    case 'readuserdefinedspec'
        T   = 0:0.01:handles.optdspec.Tmax;
        [SA]=readuserdefinedspec(T,param{:});
    otherwise
        T     = 0:0.01:handles.optdspec.Tmax;
        SA    = fun(T,param{:});
end
...
switch func2str(fun)
    ...
    case 'readuserdefinedspec', val=7; handles.codelist.Value=7; filename=param{1};
    case 'builduhs'           , val=8; handles.codelist.Value=8;
end
ds_default(handles,val,filename);
