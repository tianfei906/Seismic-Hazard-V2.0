...
ind = contains({D.name},sprintf('%s_',handles.methods(val).str));
D   = D(ind);
if ~isempty(D)
    names = regexp(strrep({D.name}','.png',''),'\_','split');
    names = vertcat(names{:});
...
[~,B]=intersect({handles.methods.label},handles.GMPEselect.String{handles.GMPEselect.Value});
str = handles.methods(B).str;
