...
load icons_MR_explorer.mat c
handles.Exit_button.CData = c{1};
handles.gridmanager.CData = c{2};
handles.openbook.CData    = c{3};
handles.AxisScale.CData   = c{4};
...
if nargin==11
    ...
    handles.mrr6 = varargin{6};
    Nmrr         = varargin{7};
    handles.mu   = varargin{8}.ShearModulus;
    Nmscl        = length(handles.mrr1);
    mscl.source  = cell(0,1);
    mscl.ptr     = zeros(0,3);
    fhand = cell(0,1);
    for i=1:Nmscl
        mscl.source = [mscl.source;handles.mrr1(i).source;handles.mrr2(i).source;handles.mrr3(i).source;handles.mrr4(i).source;handles.mrr5(i).source;handles.mrr6(i).source];
        n1 = Nmrr(1,i);
        n2 = Nmrr(2,i);
        n3 = Nmrr(3,i);
        n4 = Nmrr(4,i);
...
switch mstype
    ...
    case 2
        if ~isempty(handles.mrr2.source)
            ...
            % this is for mrr based on reasenberg analysis
            num    = handles.mrr6(ms).num(mspos,:);
            NMmin  = num(2);
            bvalue = num(5);
            Mmin   = num(6);
            Mmax   = num(7);
            handles.e1.String=sprintf('%g',NMmin);
            handles.e2.String=sprintf('%g',bvalue);
            handles.e3.String=sprintf('%g',Mmin);
            handles.e4.String=sprintf('%g',Mmax);
            handles.e5.String='-';
            handles.e6.String='-';
        end
    ...
    case 5
        ...
        handles.e14.String=sprintf(repmat('%g ',1,length(occr)),occr');
end
