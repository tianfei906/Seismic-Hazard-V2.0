function [param,rate,MRZ]=RArect_rigid(r0,source,ellipsoid,hparam)
...
if nargout==3,  allRf = cell(1,nM);end
...
if nargout==3,  allRf = vertcat(allRf{:}); end 
...
for jj=1:Ndepend
    ...
    switch str_test
        ...
        case 'GKAS2017',                param = [M,rrup,rjb,rx,ry0,ztor,dip,width,VS30,usp];
        ...
        case 'Akkar2014',               param = [M,rhyp,rjb,nan(size(M)),VS30,usp];            
            ...
            switch gmm.usp{jj}.type
                ...
                case 'cond'
                    switch func2str(gmm.usp{jj}.handle)
                        case 'MAB2019' , PARAM{jj} = {M,rrup,gmm.txt{4},gmm.txt{5},VS30,gmm.cond,param{:}}; %#ok<CCAT>
                        case 'MAL2020' , PARAM{jj} = {M,rrup,rjb,gmm.txt{4},dip,VS30 ,gmm.cond,param{:}}; %#ok<CCAT>
                        case 'MMLA2020', PARAM{jj} = {M,rrup,VS30 ,gmm.cond,param{:}}; %#ok<CCAT>
                        case 'ML2021'  , PARAM{jj} = {M,rrup,gmm.txt{4},VS30 ,gmm.cond,param{:}}; %#ok<CCAT>
                    end
            end
    end
end
