...
if ~strcmp(fname,'residuals')
    ...
    switch handles.dispmode
        case 'Sa',    plot(handles.ax1,T',IM','-','color',[1 1 1]*0.76,'tag','spec')
            plot(handles.ax1,T',mean(IM,1)','-','color',[1 0 0],'tag','spec','linewidth',2)
        case 'Sd',    plot(handles.ax1,T',IM'*981./(2*pi./T').^2,'color',[1 1 1]*0.76,'tag','spec')
            plot(handles.ax1,T',mean(IM*981./(2*pi./T).^2,1)','-','color',[1 0 0],'tag','spec','linewidth',2)
...
if strcmp(handles.display_DS.Checked,'on') && ~isempty(DS) && ~strcmp(fname,'residual')
    ...
    for j=1:Nds
        ...
        switch handles.dispmode
            case 'Sa' , plot(handles.ax1,To,ds,'r','linewidth',2,'DisplayName',label,'tag','spec');
            case 'Sd' , plot(handles.ax1,To,ds*981./(2*pi./To).^2,'r','linewidth',2,'DisplayName',label,'tag','spec');
        end
    end
end
