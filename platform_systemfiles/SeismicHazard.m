...
load icons_SeismicHazard c jp
handles.engine.CData               = c{1};
handles.Erase.CData                = c{2};
handles.Exit_button.CData          = c{3};
handles.form1                      = c{4};
handles.form2                      = c{5};
handles.HoldOn.CData               = c{6};
handles.addLeg.CData               = c{7};
handles.ax2Limits.CData            = c{8};
handles.OpenRef.CData              = c{9};
handles.ColorSecondaryLines.CData  = c{10};
handles.po_refresh_GE.CData        = c{12};
handles.RefreshButton.CData        = c{13};
handles.restore_axis.CData         = c{14};
handles.Distance_button.CData      = c{15};
...
anw = listdlg('PromptString','Default Seismicity Models:',...
    ...
    'Chile Single (Poulos et al., 2018)'...
    'Peru (SENCICO, 2016)',...
    'Mexico (2019)',...
...
switch anw
    ...
    case 3,  [handles.sys,handles.opt,handles.h]=psha_updatemodel('Chile_Poulos_Single');  house_keeping1;        
    case 4,  [handles.sys,handles.opt,handles.h]=psha_updatemodel('Peru_Default');  house_keeping1;
    case 5,  [handles.sys,handles.opt,handles.h]=psha_updatemodel('Mexico_Default_not_CU'); house_keeping1;
    case 6,  [handles.sys,handles.opt,handles.h]=psha_updatemodel('MexicoCity_CU_PreviousTectonic_2018_f2'); house_keeping1;
    case 7,  [handles.sys,handles.opt,handles.h]=psha_updatemodel('Ecuador_Default'); house_keeping1;
    case 8,  [handles.sys,handles.opt,handles.h]=psha_updatemodel('Colombia_Default'); house_keeping1;
    case 9,  handles=usgs_updatemodel(handles,'USGS_NHSM_2008');
    case 10, handles=usgs_updatemodel(handles,'USGS_NHSM_2014');
end
plot_sites_PSHA
guidata(hObject,handles)

function TestModels_Callback(hObject, eventdata, handles)
W = what('platform_defaultmodels');
D = dir(W.path);
D = D(contains({D.name},'.txt').*contains({D.name},'ST')==1);
...
if ~isempty(handles.sys)
    filename = handles.sys.filename;
    initializePSHA;
    [handles.sys,handles.opt,handles.h]=psha_updatemodel([],filename);
    house_keeping1
    plot_sites_PSHA
    handles.defaultpath_others=fileparts(filename);
    guidata(hObject, handles);
end
...
haz2txt_1(FileName,PathName,WEIGHT,handles.MRE,handles.opt,handles.h)
% haz2txt_2(FileName,PathName,WEIGHT,handles.MRE,handles.opt,handles.h)

% this if for FONDEF's Site Hazard
% WEIGHT   = handles.sys.weight(:,5);
% FileName = strrep(handles.sys.sitefilename,'.txt','.out');
% haz2txt_3(FileName,handles.MRE,WEIGHT,handles.opt,handles.h,handles.idx)
...
data     = dsha_is(handles.opt,Nsim,Nsites,handles.scenarios,handles.mulogIM,handles.L,'IS');
...
handles.opt.kmclusters = str2double(answer);

Nsim     = str2double(handles.NumSim.String);
Nsites   = size(handles.h.p,1);
data     = dsha_is(handles.opt,Nsim,Nsites,handles.scenarios,handles.mulogIM,handles.L,'KM');
fid      = fopen(FILE,'w');
data     = [data;ones(1,size(data,2))*(-999)];
fwrite(fid,data(:),'double');
fclose(fid);
...
function launch_PVSHA2_Callback(hObject, eventdata, handles)
if ~isempty(handles.sys) && ~isempty(handles.sys.isREG)
    VPSHA2(handles.sys,handles.opt,handles.h)
end

function launch_PVSHA3_Callback(hObject, eventdata, handles)
if ~isempty(handles.sys) && ~isempty(handles.sys.isREG)
    VPSHA3(handles.sys,handles.opt,handles.h)
end
...
function launch_DSPEC_Callback(hObject, eventdata, handles)
if ~isempty(handles.sys) && ~isempty(handles.sys.isREG)
    
    optdspec = createObj('optdspec',handles.sys);
    answer = questdlg('Include UHS?', ...
        'Design Spectrum', ...
        'Yes','No','Yes');
    if strcmpi(answer,'Yes')
        opt    = handles.opt;
        opt.IM = optdspec.periods;
        Nsites = length(handles.h.id);
        [optdspec.im,optdspec.lambdauhs]=runUHS(handles.sys,opt,handles.h,1:Nsites);
    end
    handles.optdspec = optdspec;
    DSPEC(handles.sys,handles.opt,handles.h,handles.optdspec)
end
...
if ispc && isfield(handles.sys,'validation') && ~isempty(handles.sys.validation)
    comparePEER(handles.sys.validation,handles.MRE,opt.im,handles.sys.weight(:,end),handles.sys.filename)
end
...
function RunEvents_Callback(hObject, eventdata, handles)
if isempty(handles.sys.event),return;end
if strcmp(handles.DSHAmode,'probabilistic') || isempty(handles.source) || isempty(handles.scenarios)
    val1       = 1;
    [handles.scenarios,handles.source]=buildeventscenario(handles.sys,handles.opt,val1);
end

handles.DSHAmode  = 'events';
Ns = size(handles.scenarios,1);
handles.pop_scenario.Value   = 1;
handles.pop_scenario.String  = {handles.sys.event.name};
handles.pop_field.String     = IM2str([handles.opt.IM1(:);handles.opt.IM2(:)]);
...
function Run_DHSA_Callback(hObject, eventdata, handles)

if strcmp(handles.DSHAmode,'events') || isempty(handles.source) || isempty(handles.scenarios)
...
handles.DSHAmode  = 'probabilistic';
handles.pop_scenario.Value = 1;
[handles.mulogIM,handles.L] = run_DSHA(handles.source,handles.scenarios,handles.h,handles.opt);
plot_DSHA
...
data     = dsha_is(handles.opt,Nsim,Nsites,handles.scenarios,handles.mulogIM,handles.L,'KM');
handles.kmdata=data;
handles.krate = data(:,1);
handles.kY    = data(:,2:end);
dsha_lambda2(handles);
...
plot_DSHA
if strcmp(handles.DSHAmode,'probabilistic')
    dsha_lambda2(handles);
end
guidata(hObject,handles)

function show_RA_Callback(hObject, eventdata, handles)
ch=findall(handles.ax1,'tag','ra');
...
i   = handles.po_region.Value;
tag_mesh = sprintf('mesh%g',handles.po_region.Value);
ch=findall(handles.ax1,'tag',tag_mesh);

switch handles.po_sourcemesh.Value
    case 0, edgecolor  = 'none';
    case 1, edgecolor  = [0.6 0.6 1];
end
switch handles.po_depth.Value
    case 0, facecolor='none'; 
    case 1, facecolor='interp';
end
set(ch,'facecolor',facecolor,'edgecolor',edgecolor);
...
function po_hypocenters_Callback(hObject, eventdata, handles)
tag = sprintf('hyp%g',handles.po_region.Value);
...
for i=1:ngeom
    ...
    tag=sprintf('hyp%g',i); ch=findall(handles.ax1,'tag',tag);set(ch,'Visible','off');
    tag=sprintf('sourcetag%g',i); ch = findall(handles.ax1,'tag',tag); set(ch,'Visible','off');
end
...
if handles.po_hypocenters.Value
    tag=sprintf('hyp%g',hObject.Value);
    ch=findall(handles.ax1,'tag',tag);
    set(ch,'Visible','on');
end
