...
load icons_SelectLocations.mat c
handles.Exit_button.CData   = c{1};
handles.gr.CData            = c{2};
handles.draw_line.CData     = c{3};
handles.draw_path.CData     = c{4};
handles.draw_point.CData    = c{5};
handles.draw_polygone.CData = c{6};
handles.po_Refresh.CData    = c{7};
handles.Distance_button.CData = c{8};
handles.draw_square.CData   = c{9};
...
if isempty(handles.opt.ellipsoid.Code)
    ...
    handles.p.ColumnName(2:3)={'X(km)';'Y(km)'};
    handles.grid_data.ColumnName(1:2)={'X(km)';'Y(km)'};
else
    xlabel(handles.ax1,'Lon','fontsize',8,'fontname','arial');
    ylabel(handles.ax1,'Lat','fontsize',8,'fontname','arial')
    handles.p.ColumnName(2:3)={'Lon';'Lat'};
    handles.grid_data.ColumnName(1:2)={'Lon';'Lat'};
end
