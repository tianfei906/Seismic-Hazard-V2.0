...
if ~isfield(handles,'lambda')
    handles.lambda    = runlogictree1(handles.sys,handles.opt,handles.h,handles.site_selection);
    handles.lambda    = permute(handles.lambda,[1,2,3,5,4]);
end
hazlevel  = 1./str2double(handles.ReturnPeriod.String);
[Nsite,~,NIM,Nbranches] = size(handles.lambda);
handles.UHS = zeros(Nsite,NIM,Nbranches);
set(handles.figure1,'Pointer','watch');
for i=1:Nsite
    for j=1:Nbranches
        lij = permute(handles.lambda(i,:,:,j),[2,3,1]);
