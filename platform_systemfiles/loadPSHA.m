function[sys,opt,h]=loadPSHA(filename,overwrite)
...
if size(opt.im,2) ~= length(opt.IM)
    opt.im = createObj('defaultIMrange',opt.IM);
end
...
%% OVERWRITES OPTIONS (Used in LIBS)
if nargin==2
   opt.MagDiscrete = overwrite.MagDiscrete;
end
...
% PRE-PROCESS ADJ MODELS
for i=1:length(GMMLIB)
    hnd = func2str(GMMLIB(i).handle);
    [~,b]=intersect(str,hnd);
    if strcmp(typ{b},'adj')
        gmm = GMMLIB(i).usp{2};
        [GMMLIB(i).T,GMMLIB(i).Rmetric,GMMLIB(i).Residuals]=mGMPE_info(gmm);
    end
end
...
if ~isnan(ptrs(10,1))
    ...
    Nevent = size(str,1);
    event(1:Nevent,1) = createObj('eventsim');
    for i=1:Nevent
