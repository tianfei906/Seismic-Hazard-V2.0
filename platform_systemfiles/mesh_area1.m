function[p,conn,area,hyp]=mesh_area1(pv,s3,lmax,nref,ellip)

% GENERATES QUADRATIC MESH OF THE 3D SOURCE GIVEN BY p
if size(pv,1)==4
    [p,conn,area,hyp] = mesh_quad(pv,lmax,nref);
    return
end
...
if max(abs(pv_rot(:,3)))>0.03*max(mean(abs(pv_rot(:,1:2)))) && s3>1e-4
    [p,conn,area,hyp]=mesh_tria_turbo(pv,lmax,nref,ellip);
    conn(:,end+1)=conn(:,3);
    return
end

%% This code handles unwarped or barely warped zones
...
conn(:,end+1)=conn(:,3); % just for graphical purposes
