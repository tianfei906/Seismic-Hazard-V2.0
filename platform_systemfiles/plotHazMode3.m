function [str,tit]=plotHazMode3(fig,haz,opt,MREPCE,idx,brnames)
...
switch haz.pce(1)
    ...
    case 0 
        ...
        st = sprintf('Percentile %g',haz.pce(3));
end
...
if haz.sbh(4)
    plot(ax2,nan,nan,'-','linewidth',2,'tag','lambda1','visible','on')
    plot(ax2,im',y1(1,:)'    ,'-','color',[1 1 1]*0.8,'ButtonDownFcn',{@myfun,fig,haz},'tag','lambda1','visible','on')
    plot(ax2,im',y1(2:end,:)','-','color',[1 1 1]*0.8,'ButtonDownFcn',{@myfun,fig,haz},'tag','lambda1','visible','on','HandleVisibility','off')
end
ax2.ColorOrderIndex=1;
plot(ax2,im',y0','-','ButtonDownFcn',{@myfun,fig,haz},'tag','lambda0','linewidth',2,'HandleVisibility','off');
ax2.Layer='top';
...
switch opt.PCE{2}
    case 'PC'
        if haz.sbh(4), str = [{st},{'Simulations (PCE)'}];  end
    case 'MC'
        if haz.sbh(4), str = [{st},{'Simulations (MCS)'}];  end
end
...
data{1,2}=st;
c2 = uicontextmenu;
uimenu(c2,'Label','Copy data (IM-Hazard)','Callback'  ,{@data2clipboard_uimenu,data(1:end,:)});
