...
for i=1:Ngeom
    ...
    if n3>0
        ...
        %patch('parent',ax,'vertices',vert3(:,1:2),'faces',sys.area1(i).conn,'facecolor','none','edgecolor',meshcolor,'tag',tag,'visible',vis);
        patch('parent',ax,...
            'vertices',vert3(:,1:2),...
            'faces',sys.area1(i).conn,...
            'facevertexcdata',vert3(:,3),...
            'facecol','none',...
            'edgecolor','none',...
            'tag',tag,'visible','on',...
            'facealpha',0.75);
    ...
    if ~isempty(sys.area1(i).txt)
        C = xyz2gps(vertcat(sys.area1(i).rclust.C),opt.ellipsoid);
        tag2 = sprintf('hyp%g',i); delete(findall(ax,'tag',tag2));
        plot(ax,C(:,1),C(:,2),'m.','tag',tag2,'visible',vis)
    end
