...
if any(strcmp({handles.me_con.label},handles.GMPEselect.String{handles.GMPEselect.Value}))
    return
end
...
[Rrup,param2] = mGMPErrupLoop(handles,param);
if isempty(param2)
...
switch uLB
    case 0, IM = IM(LB==0); LB = LB(LB==0);
    case 1, IM = IM(LB==1); LB = LB(LB==1);
    case 2, IM = IM(LB==2); LB = LB(LB==2);
    case 3, IM = IM(LB==3); LB = LB(LB==3);
end
...
condptr = [];
for i=1:numel(param)
    if isa(param{i},'function_handle')
        condptr=i;
    end
end
...
epsilon = handles.epsilon;
Neps    = length(epsilon);
[param,~,p2,t2]   = mGMPEgetparam(handles);
IM      = real(handles.IM); IM = IM(imptr);
LB      = imag(handles.IM); LB = LB(imptr);
uLB     = LB;
...
% check if conditional model is present
JND =[];
if ~isempty(t2)
    DSC =ismember(lower(t2),{'media','vs30'});
    if any(DSC)
        JND = find(DSC);
    end
end
...
str=regexp(addIMunits(IM),'\ ','split');
str=sprintf('%s / %s1000',str{1},str{1});
ylabel(handles.ax1,str,'fontsize',10);
