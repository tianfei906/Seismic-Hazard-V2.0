...
mec     = zeros(numel(source),1);
for i=1:numel(mec)
    mec(i)=source(i).numgeom(1);
end
mec     = mec(sf,1); % list of mechanisms
...
switch func2str(source.pfun)
    case 'RAcirc_leak' , param = param_circDSHA(r0,rf,M,normal,RA,gmm,media,ellip,hparam);
    case 'RAcirc_leak2', param = RAcirc_leak2(r0,source,ellip,hparam);
    case 'RAcirc_rigid', param = RAcirc_rigid(r0,source,ellip,hparam);
    case 'RArect_rigid', param = RArect_rigid(r0,source,ellip,hparam);
end
...
if strcmp(func2str(funSpatial),'none_spatial')
    h    = zeros(Nsites);
else
    h    = computeh(r0);
end
