...
for i=1:Nsource
    ...
    switch str
        ...
        case 'RAcirc_rigid'
            nC      = size(source(i).rclust.C,1);
            M       = source(i).mscl(:,1);
            nM      = numel(M);
            Nscen   = nM*nC;
            rateR   = source(i).rclust.rateR;
            rateM   = source(i).mscl(:,2);
            rate    = source(i).NMmin*(rateR*rateM'); 
            rate    = rate(:);
            mag     = repmat(M,1,nC)'; 
            mag     = mag(:);
            hypm    = repmat(source(i).rclust.C,nM,1);
            newData = [i*ones(Nscen,1),mag,hypm, nan(Nscen,3),rate];
            Data    = [Data;newData]; %#ok<AGROW>
    ...
    %     elseif source(i).obj==8
    %         % placeholder
    %         mi      = vertcat(source(i).rclust.m);
    %         Nc      = vertcat(source(i).rclust.Nc);
    %         M       = source(i).mscl(:,1);
    %         Neff    = interp1(mi,Nc,M,'nearest','extrap');
    %         jptr    = interp1(mi,(1:numel(mi))',M,'nearest','extrap');
    %         Nscen   = sum(Neff);
    %         rate    = [];
    %         for j=1:numel(M)
    %             jj    = jptr(j);
    %             rateM = source(i).mscl(j,2);
    %             rateR = source(i).rclust(jj).rateR;
    %             rate  = [rate;source(i).NMmin*rateM*rateR]; %#ok<AGROW>
    %         end
    %
    %         newData = [i*ones(Nscen,1),nan(Nscen,7),rate];
    %         Data    = [Data;newData]; %#ok<AGROW>
    %
